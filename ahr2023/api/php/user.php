...
switch ($stage) {
    case 'toggleStatus':
        ...
        $strSQL = "SELECT $target_param FROM sx4_account WHERE uid = '$target_uid'";
        $res = $db->fetch($strSQL ,false, false);
        if($res){
            $nex = 'Y';
            if($res[$target_param] == 'Y'){
                $nex = 'N';
            }

            $strSQL = "UPDATE sx4_account SET $target_param = '$nex', allow_status = 'Y' WHERE uid = '$target_uid'";
            $db->execute($strSQL);
