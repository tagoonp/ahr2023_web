...
switch($stage){
    ...
    case 'send_resetpassword':
        ...
        $strSQL = "SELECT * FROM sx4_account WHERE email = '$email' AND active_status = 'Y' AND allow_status = 'Y' AND delete_status = 'N'";
    ...
    case 'login': 
        ...
        if($res){
            ...
            if($resInsert){
                ...
                $content .= '<p>You have loged in to AHR-iCon 2023 submission system. If this activity does not done by you, please go to <a href="' . ROOT_DOMAIN . 'ahr2023/submission/">' . ROOT_DOMAIN . 'ahr2023/submission/</a> to login, check activity and change your password if nessesory.</p>';
                $content .= '<p>Regards, AHR-iCon 2023 Auto mailer system</p>';
                sendEmail(MAIL_EMAIL, MAIL_PASSWORD, MAIL_EMAIL, 'AHR-iCon 2023 Auto mailer system', $user_fullname, $res['email'], 'AHR-iCon 2023 Account login notification.', $content);

                $_SESSION['ahr2023_uid'] = $uid;
                $_SESSION['ahr2023_role'] = $res['core_role'];
                $_SESSION['ahr2023_id'] = session_id();
                $_SESSION['ahr2023_token'] = $access_token;
        ...
        if($res){
            ...
            if($resInsert){
                ...
                if($resUser){
                    ...
                    $content = '<p>Dear ' . $user_fullname . '</p>';
                    $content .= '<p>Your account is verified, please go to login at <a href="' . ROOT_DOMAIN . 'ahr2023/">' . ROOT_DOMAIN . 'ahr2023/</a></p>';
                    $content .= '<p>Regards, AHR-iCon 2023 Auto mailer system</p>';
                    sendEmail(MAIL_EMAIL, MAIL_PASSWORD, MAIL_EMAIL, 'AHR-iCon 2023 Auto mailer system', $user_fullname, $resUser['email'], 'AHR-iCon 2023 Account Verified.', $content);
                }

                 // By pass to system
                $_SESSION['ahr2023_uid'] = $uid;
                $_SESSION['ahr2023_role'] = 'participant';
                $_SESSION['ahr2023_id'] = session_id();
                $_SESSION['ahr2023_token'] = $access_token;
    ...
    case 'register' : 
        ...
        if($resInsert){
            ...
            if($resInsert){
                ...
                $content .= '<p><a href="' . ROOT_DOMAIN . 'ahr2023/api/php/authen?stage=activate&uid=' . $uid . '&sid=' . $sid .'">' . ROOT_DOMAIN . 'ahr2023/api/php/authen?stage=activate&uid=' . $uid . '&sid=' . $sid .'</a><p>';
                $content .= '<p>If you encountered any issues while confirming the email, do not hesitate to Contact Us.</p>';
                $content .= '<p>Regards, AHR-iCon 2023 Auto mailer system</p>';

                if(sendEmail(MAIL_EMAIL, MAIL_PASSWORD, MAIL_EMAIL, 'AHR-iCon 2023 Auto mailer system', $fname . " " . $lname, $email, 'AHR-iCon 2023 Account Verification', $content)){
