                            ...
                            foreach ($res['data'] as $row) {
                              ...
                              <tr>
                                ...
                                <td style="vertical-align: top;">
                                  ...
                                  <div class="pt-2">
                                    <button class="btn btn-icon btn-label-primary btn-sm" type="button" onclick="window.location = 'app-manage-submission?id=<?php echo $row['abstract_id'];?>'"><i class="bx bx-search"></i></button>  
                                    <?php
                                    if($currentUser['role_admin'] == 'Y'){
                                      ?>
                                      <button class="btn btn-icon btn-label-danger btn-sm" type="button" onclick="submission.delete_submission('<?php echo $row['abstract_id'];?>')"><i class="bx bx-trash"></i></button>  
                                      <?php
                                    }else{
                                      ?>
                                      <button class="btn btn-icon btn-label-danger btn-sm" type="button" disabled><i class="bx bx-trash"></i></button>  
                                      <?php
                                    }
                                    ?>
                                  </div>
                                ...
                                <td style="text-align: left;vertical-align: top; ">
                                  <a href="<?php echo $row['abstract_recent_file']; ?>" target="_blank"><i class='bx bx-link-external' style="font-size: 1.4em;"></i></a>
                                </td>
                                <td style="text-align: right;vertical-align: top; width: 120px;">
                                  <?php 
                                  if($row['abstract_status'] == 'wait for staff review'){
                                    ?>
                                    <span class="badge rounded-pill bg-label-secondary">Wait for assign reviewer</span>
                                    <?php
                                  }else if($row['abstract_status'] == 'wait for committee review'){
                                    ?>
                                    <span class="badge bg-label-warning">Wait for committee</span>
                                    <?php
                                  }else if($row['abstract_status'] == 'wait for reviewer review'){
                                    ?>
                                    <span class="badge bg-label-primary">Under review process</span>
                                    <?php
                                  }else if($row['abstract_status'] == 'wait for update'){
                                    ?>
                                    <span class="badge bg-label-primary">Author update</span>
                                    <?php
